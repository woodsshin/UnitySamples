...
namespace LoadTestBot
{
    internal class Program
    {
        ...
        {
            Console.WriteLine("========================================");
            Console.WriteLine(" DeterministicSampleServer 부하 테스트 봇");
            Console.WriteLine("========================================");
        ...
        private static void PrintFinalReport(LoadTestStats stats, List<BotClient> bots, int targetCount)
        {
            ...
            Console.WriteLine();
            Console.WriteLine("참고: 이 도구는 접속 종료 패킷을 보내지 않습니다. 프로그램 종료 후에도 서버는");
            Console.WriteLine("      TIMEOUT_SECONDS(3초) 동안 봇들을 살아있는 플레이어로 유지하다 정리합니다.");
        }
        ...
        private static bool TryParseArgs(string[] args, out LoadTestConfig config, out string error)
        {
            ...
            {
                // 봇개수를 생략한 경우 에러로 막지 않고 스모크 테스트 규모로 기본 실행한다.
                // 값은 줬는데 숫자가 아니거나 0 이하인 경우는 아래에서 에러로 처리한다 —
                // 오타가 기본값으로 덮어써 원치 않는 규모로 테스트가 도는 것을 막기
                // 위함이다.
